...
if (Object.keys(firebaseConfig).length > 0) {
    ...
    try {
        ...
        console.warn('Realtime DB not available', e);
    }
}
...
const saveCartToRTDB = async (userId, cartItems) => {
    ...
    try {
        ...
        await set(cartRef, { items: cartItems, updatedAt: new Date().toISOString() });
    } catch (e) {
...
const App = () => {
  ...
  const createOrder = async (cartItems, total) => {
      ...
      try {
          const ordersRef = ref(rtdb, 'pedidos');
          ...
          console.log('Pedido criado com key:', newOrderRef.key);
          setCart([]);
  ...
  useEffect(() => {
    if (!isAuthReady || !userId || !rtdb || isLoading) return; 

    const saveTimeout = setTimeout(() => {
        saveCartToRTDB(userId, cart);
    }, 500);
